...
- `study_list` - list containing all the studies that were filtered by
  the [`filter()`](https://rdrr.io/r/stats/filter.html) function.
...
- `download(output_dir)` - downloads all studies from
  [`filter()`](https://rdrr.io/r/stats/filter.html) into `output_dir`.
- `dataframe()` - loads all the studies from
  [`filter()`](https://rdrr.io/r/stats/filter.html) into a list, with
  column `study_list` that contains the names of the studies and `df`
  that contains a list of the study data information.
